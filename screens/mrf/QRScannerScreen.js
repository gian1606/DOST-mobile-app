...
export default function QRScannerScreen() {
  ...
  async function confirmIssue() {
    ...
    setError('');
    try {
      const payload = { qrValue: scannedQR, wasteTypeId: selectedWaste.id, weightKg };
  ...
  // Unmount camera when tab is not focused
  if (!isFocused) {
  ...
  return (
    <View style={styles.screen}>
      ...
      {/* Camera — only rendered when tab is focused and no QR scanned yet */}
      {!scannedQR && (
        <CameraView
          style={styles.camera}
          facing="back"
        ...
        <View style={styles.bottomSheet}>
              ...
              {wasteTypes.map((type) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                  >
                    <Ionicons name={type.icon || 'cube-outline'} size={18} color={isSelected ? colors.secondary : colors.textSecondary} />
                    <Text style={[styles.wasteOptionText, isSelected && styles.wasteOptionTextSelected]}>{type.label}</Text>
                    <Text style={[styles.wasteRate, isSelected && styles.wasteRateSelected]}>{type.eco_per_kg} ECO/kg</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
          ...
          >
            {loading
              ...
              : <><Ionicons name="diamond" size={18} color={colors.secondary} /><Text style={styles.issueBtnText}>Issue {ecoToAward > 0 ? `${ecoToAward} ` : ''}ECO</Text></>
            }
          </TouchableOpacity>
      ...
      <Modal visible={successVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.txHash} numberOfLines={1}>tx: {result?.txHash?.slice(0, 22)}...</Text>
...
const styles = StyleSheet.create({
  ...
  errorBanner:     { marginHorizontal: 20, marginBottom: 16, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: 'rgba(220,38,38,0.85)', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 10 },
